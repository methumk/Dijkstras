...
public:
    ...
    bool withinBoundary(Node *n, const sf::Vector2i &mpos, size_t node_radius)
    {
        if (n)
        {
            ...
            if (node_ys < 0)
                node_ys = 0;
            if (node_xe >= simul_width)
                node_xe = simul_width - 1;
            if (node_ye >= simul_height)
                node_ye = simul_height - 1;
    ...
    */
    ...
    void drawIMAlgoMenu(AlgoToRun &runAlgo, SimulState &state)
    {
        ...
        if (runAlgo != AlgoToRun::NoAlgo)
        {
            ...
            if (algoSelectingNode)
            {
                state = SimulState::SelectNodeMode;
            }
        }
    }
